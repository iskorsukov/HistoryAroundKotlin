...
import my.projects.historyaroundkotlin.presentation.viewstate.main.map.location.LocationErrorItem
...
class LoadingLocationFragmentTest: BaseViewModelFragmentTest<LoadingLocationFragment, MapFlowViewModel>() {
    ...
    private fun prepareErrorLocationLiveData(error: LocationErrorItem) {
        val liveData = MutableLiveData<LocationViewState>()
    ...
    fun navigatesToErrorScreenOnLocationError() {
        prepareErrorLocationLiveData(LocationErrorItem.LOCATION_UNAVAILABLE)
    ...
    fun navigatesToErrorScreenOnLocationServicesUnavailable() {
        prepareErrorLocationLiveData(LocationErrorItem.LOCATION_SERVICE_UNAVAILABLE)
