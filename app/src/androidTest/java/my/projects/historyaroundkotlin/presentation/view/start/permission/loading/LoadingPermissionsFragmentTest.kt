...
@LargeTest
class LoadingPermissionsFragmentTest: BaseViewModelFragmentTest<LoadingPermissionsFragment, PermissionFlowViewModel>() {
    ...
    private fun setupMissingLocationPermission(): List<PermissionRationale> {
        val missingPermissions = listOf(PermissionRationale(R.drawable.ic_location, R.string.location_permission_label, R.string.location_rationale))
        val missingPermissionsLiveData = MutableLiveData<PermissionViewState>()
        missingPermissionsLiveData.postValue(PermissionViewState(LoadingResultState.content(), missingPermissions))
    ...
    private fun setupNoMissingPermissions() {
        val missingPermissionsLiveData = MutableLiveData<PermissionViewState>()
        missingPermissionsLiveData.postValue(PermissionViewState(LoadingResultState.content(), emptyList()))
        doReturn(missingPermissionsLiveData).`when`(mockViewModel).checkPermissions()
    }
